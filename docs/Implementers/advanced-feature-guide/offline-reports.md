...
Custom design cards let an implementer render a fully bespoke HTML view on a dashboard tile, populated by a JavaScript data rule and localised through the standard Avni translations pipeline. Reach for this card type when standard or nested cards can't express the layout, typography, or computed values you need; otherwise prefer a simpler card type for performance and ergonomics.

### Data rule contract

The data rule is a single JavaScript expression that yields a function `({ params, imports }) => result`. Two return shapes are supported:

- **Eager:** `{ primaryValue, secondaryValue, data }` — `data` is the payload passed to the HTML template at render time.
- **Deferred:** `{ primaryValue, secondaryValue, deferredDataFunction }` — the function is invoked only when the user opens the detail view. Use this when the payload is heavy enough that you don't want to materialise it on every dashboard refresh.
...
1. **Create.** App Designer → **Report Cards → Add Report Card**. Set Card Type to **Fully custom card**.
...
4. **Upload the HTML template and save.** Pick a `.html` file. The file is a JavaScript template literal: every `${...}` is evaluated against `data` and `translations` at render time. Use `white-space:pre-wrap` on any element where multi-line translation defaults need to retain their line breaks.
   ...
   <div style="padding:12px;font-family:sans-serif;">
     ...
     <div style="font-size:12px;margin-top:4px;">
       ${translations['card.farmer.lastRegistered'].replace('{name}', data.rows[0]?.name || '—')}
     </div>
     <div style="font-size:11px;color:#999;white-space:pre-wrap;">
...
The card stores English defaults on the entity itself; Lokalise authors localised values via the existing **Translations** screen (export → translate → import → device sync). At render time on the device the localised value wins; if missing, the card's English default; if that's also missing, the literal key renders so a broken reference is visible immediately.

### Limitations

- Keys containing dots, hyphens, spaces, or starting with a digit must use bracket access in the template (`translations['card.foo.bar']`), not dot access.
- A non-object `data` (primitive, `null`, `undefined`, or function) is silently replaced with `{}` — `${data.x}` becomes blank with no error. Wrap scalars as `{ value: scalar }` if needed.
